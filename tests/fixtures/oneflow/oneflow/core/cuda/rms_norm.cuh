...
__global__ void RmsNormWarpImpl(LOAD load, STORE store, const int nrow, const int ncol,
  ...
  const int num_row_groups = (nrow + rows_per_access - 1) / rows_per_access;
  for (int row_i = global_thread_group_id; row_i < num_row_groups;
       row_i += num_global_thread_groups) {
...
cudaError_t LaunchRmsNormWarpImpl(cudaStream_t stream, LOAD load, STORE store, const int64_t nrow,
  ...
  const int64_t num_row_groups = (nrow + rows_per_access - 1) / rows_per_access;
  const int64_t num_blocks =
      (num_row_groups + thread_groups_per_block - 1) / thread_groups_per_block;
  int grid_dim_x;
  {
